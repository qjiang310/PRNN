...
{
public:
    typedef TileConfig<1, 384, 384, 128, 256, 6, 36, direction, T> TileSize; // FIXME 
...
size_t getMaximumSizeRNNForThisGPU(const matrix::Precision& precision)
{
    ...
    size_t maxSize =  detail::TileSizeSelector(major, minor, smCount, precision).getMaximumSize();
    util::log("RecurrentOperations") << "major " << major
            << ", minor " << minor << ", sms "
            << smCount << ", max size is " << maxSize << "\n" ;
    
    return maxSize; // FIXME  
}
...
void dispatchForwardPropRecurrent(typename ArchitectureConfig::RealType* activations,
    typename ArchitectureConfig::RealType* activations_u,
    const typename ArchitectureConfig::RealType* weights,
    const typename ArchitectureConfig::RealType* weights_u,
    const typename ArchitectureConfig::RealType* weights_h,
    typename ArchitectureConfig::RealType* scratch, typename ArchitectureConfig::RealType* scratch_u, 
    const ArchitectureConfig& archParameters)
{
    ...
    while(synchronizer.not_finished()) {
        PersistentEngineParameters<Config> parameters(config, weights, weights_u, weights_h, activations, activations_u,
            scratch, scratch_u, archParameters.handle.skipConnectionScale, synchronizer);
    ...
    const std::tuple<T>& precision)
{
    ...
    const RealType* weightsData_u    = weights_u.data<RealType>();
    const RealType* weightsData_h    = weights_h.data<RealType>();
          RealType* activationData   = activations.data<RealType>();
          RealType* activationData_u = activations_u.data<RealType>();
          RealType* scratchData      = scratch.data<RealType>();
          RealType* scratchData_u    = scratch_u.data<RealType>();
    ...
    if(major == 6 && smCount >= 60)
    {
        ...
        dispatchForwardPropRecurrent<ActivationFunction, ArchParams>(activationData, activationData_u, weightsData,
            weightsData_u, weightsData_h, scratchData, scratchData_u, architectureConfig);
    }
    ...
    const std::tuple<T>& precision)
{
    ...
    if(handle.direction == prnn::RECURRENT_REVERSE)
    {
        forwardPropRecurrent<ActivationFunction, PossiblePrecision, prnn::RECURRENT_REVERSE>(
            activations, activations_u, weights, weights_u, weights_h, scratch, scratch_u, handle, precision);
    }
    else
    {
    ...
    const Precisions& precisions)
{
    ...
    if(activations.precision() == PossiblePrecision())
    {
        forwardPropRecurrentOverPrecisions<ActivationFunction>(
            activations, activations_u, weights, weights_u, weights_h, scratch, scratch_u, handle, std::tuple<PossiblePrecision>());
    }
    else
    {
        typedef typename util::RemoveFirstType<Precisions>::type RemainingPrecisions;

        forwardPropRecurrentOverPrecisions<ActivationFunction>(activations, activations_u, weights, weights_u, weights_h, scratch, scratch_u, handle, RemainingPrecisions());
    }
}
...
void forwardPropRecurrentOverPrecisions(const matrix::DynamicView& activations,
    ...
    const matrix::ConstDynamicView& weights_u,
    const matrix::ConstDynamicView& weights_h,
    const matrix::DynamicView& scratch, const matrix::DynamicView& scratch_u, 
    const RecurrentOpsHandle& handle)
{
    forwardPropRecurrentOverPrecisions<ActivationFunction>(activations, activations_u, weights, weights_u, weights_h, scratch,
        scratch_u, handle, prnn::matrix::RecurrentPrecisions());
}
    ...
    const std::tuple<ActivationFunction>& activationFunction)
{
    ...
    forwardPropRecurrentOverPrecisions<ActivationFunction>(activations, activations_u, weights, weights_u, weights_h, scratch, scratch_u, handle);
}
    ...
    const Functions& functions)
{
    ...
    {
        forwardPropRecurrentOverActivationFunctions(activations, activations_u, weights, weights_u, weights_h,
            scratch, scratch_u, handle, std::tuple<PossibleFunction>());
    }
    else
    {
        typedef typename prnn::util::RemoveFirstType<Functions>::type RemainingFunctions;

        forwardPropRecurrentOverActivationFunctions(activations, activations_u, weights, weights_u, weights_h, scratch, 
                                                    scratch_u, handle, RemainingFunctions());
    }
}

void forwardPropRecurrentOverActivationFunctions(const matrix::DynamicView& activations,
    const matrix::DynamicView& activations_u,
    const matrix::ConstDynamicView& weights, const matrix::ConstDynamicView& weights_u,
    const matrix::ConstDynamicView& weights_h,
    const matrix::DynamicView& scratch, const matrix::DynamicView& scratch_u,
    const RecurrentOpsHandle& handle)
{
    forwardPropRecurrentOverActivationFunctions(activations, activations_u, weights, weights_u, weights_h, scratch, scratch_u,
    ...
    const RecurrentOpsHandle& handle)
{
    ...
    assert(activations.precision() == weights_u.precision());
    assert(activations.precision() == weights_h.precision());
    ...
    else if(backend == RECURRENT_PERSISTENT_BACKEND)
    {
        ...
        copy(activations_u, inputActivations);
        
        detail::forwardPropRecurrentOverActivationFunctions(activations, activations_u,
            reshape(weights, {handle.layerSize, handle.layerSize}),
            reshape(weights_u, {handle.layerSize, handle.layerSize}),
            reshape(weights_h, {handle.layerSize, handle.layerSize}),
            scratch, scratch_u, handle);
    }
    else
    {
        copy(activations, inputActivations);
    ...
    typename ArchitectureConfig::RealType* scratch, const ArchitectureConfig& archParameters)
{
    ...
    while(synchronizer.not_finished()) {
        PersistentEngineParameters<Config> parameters(config, weights, weights, weights, activations, activations,
            scratch, scratch, archParameters.handle.skipConnectionScale, synchronizer);
