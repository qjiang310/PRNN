...
#define dprintf(...) { if((blockIdx.x == 0 && blockIdx.y == 0 ) && ((threadIdx.x == 0 && threadIdx.y == 0) || (threadIdx.x == 1 && threadIdx.y == 0))) \
      { std::printf(__VA_ARGS__); } }

#define t0printf(...) { if((blockIdx.x == 0 && blockIdx.y == 0 ) && ((threadIdx.x == 0 && threadIdx.y == 0) || (threadIdx.x == 1 && threadIdx.y == 0))) \
     { std::printf(__VA_ARGS__); } }
...
public:
    PersistentEngineParameters(const Config& config,
        const RealType* weights, const RealType* weights_u, const RealType* weights_h,
        RealType* activations,
        RealType* activations_u,
        RealType* activations_scratch,
        RealType* activations_scratch_u,
        double skip_connection_scale, const Synchronizer& synchronizer_)
    : PersistentEngineParameters(config, weights, weights_u, weights_h, nullptr, activations, activations_u,
        activations_scratch, activations_scratch_u, skip_connection_scale, synchronizer_)
    {
    ...
    PersistentEngineParameters(const Config& config,
        ...
        RealType* activations_or_deltas_u,
        RealType* activations_scratch,
        RealType* activations_scratch_u,
        double skip_connection_scale, const Synchronizer& synchronizer_)
    : weights(weights),
      ...
      activations_u(activations_or_deltas_u),
      activation_scratch(activations_scratch),
      activation_scratch_u(activations_scratch_u),
...
public:
    ...
    // TODO these two functions may need to by modified when doing backward propagation
...
public:
    ...
    const RealType* weights_u;
    const RealType* weights_h;
    RealType*       back_prop_activations;
    RealType*       activations;
    RealType*       activations_u;
    RealType*       activation_scratch;
    RealType*       activation_scratch_u;
          ...
          reduction_threads_per_value(parameters.reduction_threads_per_value)
        {
            ...
            if (Config::DIRECTION == prnn::RECURRENT_REVERSE) {
                ...
                input_base_pointer = parameters.activations +
                    iteration * parameters.layer_size;
                input_base_pointer_u = parameters.activations_u +
                    iteration * parameters.layer_size;

                activation_scratch = parameters.activation_scratch +
                    Config::EXPANDED_GRID_TILE_COLUMNS * iteration;
                activation_scratch_u = parameters.activation_scratch_u +
                    Config::EXPANDED_GRID_TILE_COLUMNS * iteration;
            }
            else {
                back_prop_activation_base_pointer = parameters.back_prop_activations +
...
public:
    __device__ inline void run_forward()
    {
        ...
        t0printf("Thread (%d, %d, %d, %d) - (u)Starting persistent forward engine on "
            "iteration %d (scratch %p) (acts/deltas %p).\n",
            blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, parameters.first_iteration,
            parameters.activation_scratch_u, parameters.activations_u);
        ...
        DataLoadingBuffer data_buffer_u;
        ThreadTileAccumulators accumulators; // 9 regs
        ThreadTileAccumulators accumulators_u; 
        ThreadTileOutputAccumulators output_accumulators; // 3 regs
        ThreadTileOutputAccumulators output_accumulators_u;

        RegisterState register_state(parameters);

        __shared__ SharedDataStorage shared_state;
        __shared__ SharedDataStorage shared_state_u;

        initialize_shared_state(shared_state);
        initialize_shared_state(shared_state_u);

        ThreadTileWeights weights;
        ThreadTileWeights weights_u;
        ThreadTileWeights weights_h;

        load_weights(weights, weights_u, weights_h);

        warm_start(register_state, shared_state, shared_state_u, weights, weights_u, weights_h, 
                    data_buffer, data_buffer_u, accumulators, accumulators_u, output_accumulators, output_accumulators_u);
        ...
        if(register_state.barrier_success)
        {
            for(; register_state.iteration < register_state.iterations; ++register_state.iteration)
            {
                ...
                perform_iteration(register_state, shared_state, shared_state_u, weights, weights_u, weights_h, 
                    data_buffer, data_buffer_u, accumulators, accumulators_u,
                    output_accumulators, output_accumulators_u);
        ...
        if(register_state.barrier_success)
        {
            clean_up(register_state, shared_state, shared_state_u, weights, weights_u, weights_h,
                 data_buffer, data_buffer_u, accumulators, accumulators_u,
                 output_accumulators, output_accumulators_u);
        }
...
public:
    __device__ inline void run_back_prop_deltas()
    {
        ...
        ThreadTileWeights weights;
        ThreadTileWeights weights_u;
        ThreadTileWeights weights_h; // FIXME

        load_transposed_weights(weights, weights_u, weights_h);
...
private:
    __device__ void perform_iteration(RegisterState& register_state,
        SharedDataStorage& shared_state, SharedDataStorage shared_state_u,
        ThreadTileWeights& weights, ThreadTileWeights& weights_u, ThreadTileWeights& weights_h,
        DataLoadingBuffer& data_buffer,
        DataLoadingBuffer& data_buffer_u,
        ThreadTileAccumulators& accumulators,
        ThreadTileAccumulators& accumulators_u,
        ...
        bool stage_three = true)
    {
        ...
        ThreadTileInputs thread_inputs_u;
        ...
        if(stage_one)
        {
            if(!check_for_critical_barrier_failure(register_state, shared_state) 
               || !check_for_critical_barrier_failure(register_state, shared_state_u))
            {
                return;
            }
            // dprintf("shared_state.data[4672] = %f\n", shared_state.data[4672]); // FIXME
        }

        // 0
        if(stage_one)
        {
            load_input(register_state, data_buffer, load_output, false); // load "input" activation into data_buffer
            load_input(register_state, data_buffer_u, load_output, false, true); 
            // dprintf("shared_state.data[4672] = %f\n", shared_state.data[4672]); // FIXME
        }
        ...
        if(stage_two)
        {
            load_thread_tile_inputs(register_state, shared_state, thread_inputs); // load "output" activation from shared memory to thread_inputs 
            load_thread_tile_inputs(register_state, shared_state_u, thread_inputs_u); 
            initialize_accumulators(accumulators); // initialize accumulators to zero
            initialize_accumulators(accumulators_u);
            perform_thread_tile_math(accumulators, weights, thread_inputs); // weights * thread_inputs
            perform_thread_tile_math(accumulators_u, weights_u, thread_inputs_u);         
        }

        // 2
        if(stage_three)
        {
            reduce_thread_tile_shared(register_state, shared_state, output_accumulators); // output_accumulators = current + shared_memory
            reduce_thread_tile_shared(register_state, shared_state_u, output_accumulators_u); 
            // dprintf("after reduce_thread_tile_shared shared_state.data[%d] = %f.\n", shared_offset, shared_state.data[shared_offset]);            
        }

        // 2
        if(stage_three)
        {
            store_accumulators(register_state, output_accumulators); // atomic_increment accumulators to register_state.activation_scratch
            store_accumulators(register_state, output_accumulators_u, true); 
            // dprintf("after store_accumulators shared_state.data[%d] = %f.\n", shared_offset, shared_state.data[shared_offset]);
        }
        ...
        if(stage_two)
        {
            ...
            initialize_output_accumulators(register_state, shared_state_u, output_accumulators_u);
            // dprintf("initialize_output_accumulators shared_state.data[%d] = %f.\n", shared_offset, shared_state.data[shared_offset]);
        }

        advance_shared_pointers(register_state); // to make a ping-pong buffer, read one half and write the other half

        // 1
        if(stage_two)
        {
            store_accumulators_to_shared(register_state, shared_state, accumulators); // store accumulators to shared
            store_accumulators_to_shared(register_state, shared_state_u, accumulators_u);
        }
        ...
        if(stage_one)
        {
            ...
            wait_for_barrier(register_state, shared_state_u, data_buffer_u);
            // dprintf("shared_state.data[4672] = %f\n", shared_state.data[4672]); // FIXME
        }
        ...
        if(stage_one)
        {
            format_input(register_state, shared_state, data_buffer, save_input,false);
            format_input(register_state, shared_state_u, data_buffer_u, save_input,true);
            // dprintf("shared_state.data[4672] = %f\n", shared_state.data[4672]); // FIXME
        }
        synchronize_block();
...
private:
    __device__ void warm_start(RegisterState& register_state,
        SharedDataStorage& shared_state, SharedDataStorage& shared_state_u,
        ThreadTileWeights& weights, ThreadTileWeights& weights_u, ThreadTileWeights& weights_h,
        DataLoadingBuffer& data_buffer, DataLoadingBuffer& data_buffer_u,
        ThreadTileAccumulators& accumulators,
        ThreadTileAccumulators& accumulators_u,
        ThreadTileOutputAccumulators& output_accumulators,
        ThreadTileOutputAccumulators& output_accumulators_u) {
...
private:
    __device__ void clean_up(RegisterState& register_state,
        SharedDataStorage& shared_state,
        SharedDataStorage& shared_state_u,
        ThreadTileWeights& weights,
        ThreadTileWeights& weights_u,
        ThreadTileWeights& weights_h,
        DataLoadingBuffer& data_buffer,
        ...
        for(; register_state.iteration < register_state.iterations + parameters.mini_batch_size;
            ...
            perform_iteration(register_state, shared_state, shared_state_u, 
                weights, weights_u, weights_h, data_buffer, data_buffer_u, 
                accumulators, accumulators_u, output_accumulators, output_accumulators,
...
private:
    __device__ void initialize_shared_state(SharedDataStorage& shared_state)
    {
        ...
        /*t0printf("Thread (%d, %d, %d, %d) - SHARED_BARRIER_OFFSET %d, SHARED_BUFFER_SIZE %d, shared_state.data[%d] = %f\n",
                blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, Config::SHARED_BARRIER_OFFSET, Config::SHARED_BUFFER_SIZE,
                4672, shared_state.data[4672]);*/
...
private:
    ...
    __device__ void populate_scratch() {
        ...
        for (index_t offset = id;
            ...
            RealType value = 0.0;
            RealType value_u = 0.0;

            if (is_in_range) {
                value = parameters.activations[local_index];
                value_u = parameters.activations_u[local_index];
            }

            if (is_barrier_id(offset)) {
                value = Config::THREADS_PER_GLOBAL_REDUCTION;
                value_u = Config::THREADS_PER_GLOBAL_REDUCTION;
            }
            ...
            dprintf("Thread (%d, %d, %d, %d) - Warm starting scratch "
                "offset[%d] (%p) = activations_u[%d] (%p) (%f) \n",
                // __FILE__, __LINE__, 
                blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y,
                scratch_index, parameters.activation_scratch_u + scratch_index,
                local_index, parameters.activations_u + local_index, (float)value_u);

            atomic_store_relaxed(parameters.activation_scratch[scratch_index], value);
            atomic_store_relaxed(parameters.activation_scratch_u[scratch_index], value_u);
        }
    }
...
private:
    __device__ void load_weights(ThreadTileWeights& weights,ThreadTileWeights& weights_u,
                                 ThreadTileWeights& weights_h) {
        safe_load_weights(weights, weights_u, weights_h, false);
    }

    __device__ void safe_load_weights(ThreadTileWeights& weights, ThreadTileWeights& weights_u,
                                      ThreadTileWeights& weights_h, bool transpose) {
        ...
        for (index_t column_segment = 0, data_column = 0;
            ...
            for (index_t column_offset = 0; column_offset < Config::VALUES_PER_SHARED_LOAD;
                ...
                for (index_t row = 0; row < Config::THREAD_TILE_ROWS; ++row) {
                    ...
                    weights_u.data[row][column] = 0.0;
                    weights_h.data[row][column] = 0.0;
                    ...
                    if (is_in_range) {
                        ...
                        weights_u.data[row][column] = parameters.weights_u[thread_tile_index];
                        weights_h.data[row][column] = parameters.weights_h[thread_tile_index];
                        // FIXME add t0 back
                        ...
                        // printf("%fi\n", weights.data[row][column]);
                        // printf((row+1)%4 == 0 ? "\n" : " ");
                        t0printf("Thread (%d, %d, %d, %d) - Loading thread weights[%d][%d] = "
                            "data_weights_r[%d][%d] %f, data_weights_u[%d][%d] %f, data_weights_h[%d][%d] %f\n",
                            blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y,
                            (int)row, (int)column, (int) current_row, (int)current_column,
                            (float)weights.data[row][column], (int) current_row, (int)current_column,
                            (float)weights_u.data[row][column], (int) current_row, (int)current_column,
                            (float)weights_h.data[row][column]);
                    }
                }
    ...
    __device__ void load_transposed_weights(ThreadTileWeights& weights, ThreadTileWeights& weights_u, ThreadTileWeights& weights_h)
    {
        safe_load_weights(weights, weights_u, weights_h, true);
    }

private:
    __device__ void format_input(RegisterState& register_state,
        SharedDataStorage& shared_state,
        DataLoadingBuffer& data_buffer, bool save_input, bool is_u=false)
    {
        ...
        if(save_input)
        {
            store_nonlinear_input_global(register_state, data_buffer, is_u);
        }
    ...
    __device__ void load_input(RegisterState& register_state, DataLoadingBuffer& data_buffer,
        bool load_output = true, bool is_retry = false, bool is_u = false) {

        UNROLL
        for (index_t i = 0; i < Config::GLOBAL_VALUES_PER_THREAD;
            i += Config::VALUES_PER_GLOBAL_LOAD) {

            predicated_load_vector(register_state, data_buffer.data[i], i, load_output, is_retry, is_u);
        }
    }
        ...
        DataLoadingBuffer& data_buffer)
    {
        ...
        #endif
        // dprintf("Thread (%d, %d, %d, %d) - End of wait_for_barrier: shared_state.data[%d] = [%f] \n",
        //         blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, shared_offset, shared_state.data[shared_offset]);
                // FIXME
    }
        ...
        bool is_retry = false)
    {
        ...
        {
            // dprintf("Thread (%d, %d, %d, %d) - Detect_barrier_success: shared_state.data[%d] = [%f] \n",
            //     blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, shared_offset, shared_state.data[shared_offset]);
                // FIXME
            register_state.barrier_success &=
    ...
    __device__ void store_nonlinear_input_global(RegisterState& register_state,
        DataLoadingBuffer& data_buffer, bool is_u=false)
    {
        UNROLL
        for (index_t i = 0; i < Config::USEFUL_GLOBAL_VALUES_PER_THREAD;
            i += Config::VALUES_PER_GLOBAL_STORE)
        {
            predicated_store_vector(register_state, data_buffer.data[i], i, is_u);
        }
    }
        ...
        SharedDataStorage& shared_state)
    {
        ...
        // t0printf("Barrier success = shared_state.data[%d] [%f] == 0.0.\n", shared_offset, shared_state.data[shared_offset]); // FIXME
...
private:
    ...
    __device__ void predicated_load_vector(RegisterState& register_state,
        RealType& value, index_t value_offset, bool load_output, bool is_retry, bool is_u)
    {
        ...
        RealType* load_base_r = is_input_thread() ?
        ...
        RealType* load_base_u = is_input_thread() ?
            register_state.activation_scratch_u : // might be a pointer to global memory
            (Config::DIRECTION == prnn::RECURRENT_REVERSE ?
                register_state.input_base_pointer_u - register_state.input_to_output_offset :
                register_state.input_base_pointer_u + register_state.input_to_output_offset);

        RealType* load_base = is_u ? load_base_u : load_base_r;
        ...
        for(int i = 0; i < Config::VALUES_PER_GLOBAL_LOAD; ++i)
        {
            if(condition)
            { // FIXME put d back 
                printf("Thread (%d, %d, %d, %d) - Loading %s activation[%d] "
        ...
        const RealType& accumulator, index_t value_offset)
    {
        ...
        // dprintf("Storing %f to shared_state.data[%d].\n", shared_state.data[shared_offset], shared_offset); // FIXME
    }
    ...
    __device__ void predicated_store_vector(RegisterState& register_state,
        RealType& data, index_t value_offset, bool is_u=false)
    {
        RealType* output_base = get_output_pointer(register_state, is_u);
        ...
        const RealType& value, index_t value_offset, bool is_retry) const
    {
        ...
        if (is_barrier_id(offset)) {
            dprintf("Thread (%d, %d, %d, %d) - Checking barrier counter %f (%p) against "
                "requirement %f (%s)\n",
                blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y,
                (float)value, offset, (float)register_state.reduction_threads_per_value,
...
private:
    ...
    __device__ void store_accumulators(RegisterState& register_state,
        ThreadTileOutputAccumulators& accumulators, bool is_u = false)
    {
        #if REDUCE_ADDRESS_MATH
        RealType* outputBuffer = is_u ? register_state.activation_scratch_u : register_state.activation_scratch;
    ...
    __device__ RealType* get_output_pointer(RegisterState& register_state, bool is_u=false) {
        return is_u ? register_state.input_base_pointer_u : register_state.input_base_pointer;
    }
...
private:
    __device__ void advance_pointers(RegisterState& register_state) {
        ...
        register_state.input_base_pointer_u += register_state.iteration_step;

        register_state.activation_scratch += register_state.scratch_step_size;
        register_state.activation_scratch_u += register_state.scratch_step_size;
    }
