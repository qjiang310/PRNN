...
void benchmarkRnnForward(size_t iterations, size_t layerSize, size_t miniBatchSize,
    ...
    auto weights_u    = prnn::matrix::rand({layerSize, layerSize               }, precision);
    auto weights_h    = prnn::matrix::rand({layerSize, layerSize               }, precision);
    // auto weights       = prnn::matrix::rand({layerSize, layerSize               }, precision);
    auto activations   = prnn::matrix::rand({layerSize, miniBatchSize, timesteps}, precision);
    auto activations_u = prnn::matrix::rand({layerSize, miniBatchSize, timesteps}, precision);
    ...
    auto scratch_u = prnn::rnn::getForwardPropScratch(handle, precision);
    ...
    prnn::rnn::forwardPropRecurrent(
        ...
        prnn::matrix::DynamicView(activations_u),
        prnn::matrix::ConstDynamicView(inActivations),
        prnn::matrix::ConstDynamicView(weights),
        prnn::matrix::ConstDynamicView(weights_u),
        prnn::matrix::ConstDynamicView(weights_h),
        prnn::matrix::DynamicView(scratch),
        prnn::matrix::DynamicView(scratch_u),
