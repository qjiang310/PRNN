...
def add_nvcc_flags(env):
  ...
  # env.AppendUnique(NVCCFLAGS = '-gencode=arch=compute_80,code=sm_80')
  ...
  env.AppendUnique(NVCCFLAGS = '-cudart=shared') # for GPGPU-Sim FIXME
  # env.AppendUnique(NVCCFLAGS = '-lcudart')
  env.AppendUnique(NVCCFLAGS = '-L/p/hal/private/installs/cuda-8.0/lib64')   
  env.AppendUnique(NVCCFLAGS = '-lcublas_static') 
  env.AppendUnique(NVCCFLAGS = '-lcudnn_static')
